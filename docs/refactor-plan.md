...
| 2026-08-15 | **Phase 3.1 + 3.2 — lock ADR (α) + env registry** (team `team_20260815035025_1d1e3c907f4fc244`) | `259f9312` | **3.1 ADR (α)**: event-log lock family stays separate (v0.9.26 split + 3.6 empirical); agents-record joins token-guarded pattern (randomUUID token + releaseAgentsLock, missing/corrupt safe); per-family staleMs kept; L1→L2→L3 invariant documented. **3.2 env registry**: src/config/env-vars.ts (134 names, mirror precedence crew/teams/or, behavior-preservation contract); codemod 58 dot + ~10 bracket sites/28 files → getCrewEnv; new check:env-vars gate wired into ci; 17 unit tests. Gates: 101/101 + unit 6585 + ci green (bundle 2.85 MB). Test-engineer worker crashed — leader verified directly. |
| 2026-08-15 | **Phase 3.6–3.8 — event-log dual-namespace + stranding + silent-skip (R16-18, P0/P1)** (team `team_20260815005742_4403c55b479379b9`) | `3a536e29`, `6adc61a1` | **W1 (.seqlock L3)**: shared 3rd lock namespace serializing .seq reservation cho cả 2 families; advance-on-reserve persist INSIDE lock (b9 bench bắt regression bản nháp đầu — 0 dups sau fix). NOT naive merge (giữ v0.9.26 split); ordering L1→L2→L3. **W2 (archive-tail)**: readEvents/readEventsCursor drain archive tails trên .gen bump (seq>sinceSeq, dedup) — stranded events không bị sweep mất; stale comment corrected. **W3 (R17-S1 HIGH)**: size-limit skip → skippedDueToSize indicator (NO throw sync path), severity "error" logs, rotation booleans checked 3 sites, emitFromTeamEvent gated. **W4 (R17-B1 HIGH)**: removeStaleAgentsLock catch → logInternalError warn. **W6 (LOW)**: pi-spawn + subagent-manager catches. **Durable bench** `bench/b9-eventlog-dual-namespace.bench.ts`: effect1 300 events/2 processes → **0 dup seq** (trước: 75%); effect2 250/250 recovered (9 archive-stranded) + 8 unit regression tests. **Phase 3 phase-gate `npm run ci` GREEN**: unit 6556 (0 fail), integration 192 (0 fail), bundle 2.82 MB. |
