...
## Addendum 2 — F1 fix campaign (23:50–00:50+07): 2 commits + forensic closure

**f12f4f5d (F1 part 1)**: `onSurfaceActivity` bridge — child-pi.ts surface branch forwards every tailed per-agent event to a new input callback; child-executor reuses the stdout-path persisters (persistHeartbeat 1s / persistChildProgress coalesced) + pins worker.started pid into task.heartbeat. Live-verified ×2 (SURFACE-LIVE-1/2): task.progress flows (0→4+), pid pinned, **0 heartbeat_dead** (watcher pid-gate effective — W1 had 2).

**af2f8eb4 (F1 part 2)**: reconciler PID-liveness gate in isTaskHeartbeatStale (parity with watcher) — the surface recorder only flushes at turn boundaries; a 6.7-min LLM turn (observed: bash result ts 17:13:20 flushed to disk 17:20:00) freezes lastSeenAt mid-turn while the worker is alive. 3 unit tests (alive-pid shield / dead-pid repair / no-pid legacy).

**7372af37**: forensic sidecar (PI_CREW_DEBUG_STALE=1 → /tmp/pi-crew-f1-debug.log) recording every STALE verdict (taskId/pid/pidAlive/ages).

**SURFACE-LIVE-3 quyết định**: sidecar EMPTY + reconcile vẫn nổ ("repaired 3", run team_20260910173654, pid 2433940 pinned, progress 5, 0 heartbeat_dead) → scanner chạy trong process KHÔNG có DEBUG env → **session chính (main pi, start 00:04 — load bundle TRƯỚC f12f4f5d) là hung thủ**: reconciler cũ (không pid-gate, không bridge) quét định kỳ mọi runs trong .crew/state và giết run khỏe của session khác trên ngưỡng 5 phút. Giải thích nhất quán cả 3 lần live-run chết (17:18 / 17:33 / 17:43).
