...
| F05+F18+F19+F20 — Test/CI harness integrity (RR-015) | `docs/stories/RR-015.md` | yes | no | yes (ci.yml + weekly-smoke) | implemented | test-runner-exit.test.ts (NEW 13 tests), test-changed-mode.test.ts (NEW 8), argv-flags.smoke.ts rewritten, benchmark.test.ts 18→29; wrapper SIGKILL exit 0→1 confirmed live in full-suite run. **Mutation RE-VERIFIED 2026-09-22 (RM-04)**: reverted `resolveExitCode` to `result?.status ?? 0` → 6/13 RED incl. AC-1 (end-to-end wrapper SIGKILL) and AC-5; restored → 13/13 GREEN. |
