...
**Implementation — keep coalescing, drop only durability (controller correction after T3 review):** the coalesced entry already stores `durability` per-file (atomic-write.ts:980) and forwards it at flush (:997), so the flag can be honored WITHOUT bypassing coalescing: when `persistence.skipTasksFsync === true` AND the save is *not* skipping coalescing (non-terminal checkpoint), pass `{ compact: true, durability: "best-effort" }` into the coalesced call so the pending entry's durability is best-effort — the 50ms coalescing (RMW grouping) is preserved, and at flush the single atomicWriteFile skips both fsyncs. Do NOT bypass the coalesced path (that would trade coalescing for no-fsync and add a synchronous write per save). Terminal transitions keep `skipCoalesce: true` → `atomicWriteJson(..., durability: "full")` untouched — the flag never touches the terminal path.
