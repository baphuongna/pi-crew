...
**Revision-2 producer surface (deferred — code review R6):** no shipped action mints a revision v2 yet (`orchestrate` binds v1; adaptive injects once; the planner producer fills an empty store only). The revision machinery (append lineage, copy-forward, diff, dropped-item sweep) is therefore exercised via store APIs and tests until a `plans revise` producer lands (follow-up). Recorded here so §4's re-plan semantics are not read as reachable end-to-end in T2.

**UI readers stay manifest-only (code review R3):** scheduler gates (dispatch, reconciler, crash-recovery, approve command) are record-first via `isPlanApprovalPendingEffective`; UI surfaces (widget/powerbar/dashboard) keep reading `manifest.planApproval` — dual-write keeps the sides consistent, and per-render plans.json reads would tax hot UI paths. The manifest field is deprecated for *writers*, never dropped for these readers.
