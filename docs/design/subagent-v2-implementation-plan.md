...
**Steps:** 1) ADR-5 merge 2) config plumbing FIRST: `nestingEnabled` (default **false** in T3 until WP-5 completes) in `src/config/defaults.ts` + `src/schema/config-schema.ts` (+ `config-schema-sync` update); spawn-policy + broker `delegate` read it; disabled → structured policy rejection + `events.jsonl` log + negative flag-off tests 3) spawn-policy + nested-slots (+ unit tests incl. deadlock negative: 4-core box, 2 concurrent delegates complete; budget exhausted → immediate reject, never queue) 3) depthOverride + issuer gate 4) broker delegate + grandchild lifecycle 5) allocation accounting + roll-up 6) prompt-runtime tool 7) extension allowlist 8) serialize-on-overlap wiring. 9) ADR-5 pin (vii): worker-token TTL/rotation decision (or accepted-risk entry with revisit date) — design §11 P2-20 not dropped.
...
- **T2/WP-4:** NEW `src/state/stores/plan-store.ts`, `src/state/types.ts`, `src/runtime/goal-workflow/adaptive-plan.ts`, `src/extension/plan-orchestrate.ts`, `src/runtime/plan-approval.ts`, `src/extension/team-tool/api/plan-approval.ts`, NEW `src/extension/team-tool/plans.ts`, `src/extension/team-tool.ts` + `src/schema/team-tool-schema.ts` (action registration + count tests), `src/runtime/team-runner.ts` (:808,1055 readers), `src/runtime/stale-reconciler.ts` (:46 reader).
- **T3/WP-5:** NEW `src/runtime/spawn-policy.ts`, NEW `src/runtime/scheduling/nested-slots.ts`, `src/runtime/child-pi/child-pi.ts` (depthOverride seam :253-257,277-285), `src/runtime/broker/crew-broker.ts` (delegate handler), `src/state/types.ts` (`task.depth`,`allocation`), `src/runtime/budget-enforcement.ts`, `src/runtime/model/pi-args.ts` (:306-331 allowlist), `src/runtime/deadletter.ts` + heartbeat observers (grandchild registration), `src/config/defaults.ts` + `src/schema/config-schema.ts` (`nestingEnabled`), `src/state/contracts.ts`.
- **T4/WP-6:** NEW `src/state/stores/spec-store.ts`, `src/runtime/task-packet.ts`, `src/runtime/task-runner/pre-execution.ts`, `src/runtime/task-runner/post-execution.ts`, `src/runtime/task-runner/prompt-builder.ts` (spec section), `src/state/types.ts`, + skills dir (`requirements-to-task-packet`, postinstall refresh).
