 ...
 * subagent: run-graph build/save and event-log growth estimates for N
 * subagents.
 *
 * HISTORY: this bench previously also measured the ObservationStore
 * (record + save + load). That module was removed as dead code in 51f46915
 * ("refactor(dead-code): Phase 1.1-1.8 sweep — remove 92 dead modules") with
 * no replacement facility anywhere in src/ — do NOT resurrect it. The
 * observation cases were dropped; the surviving subjects below are measured
 * against the current modules.
 *
 * Sizes: 1, 10, 50 subagents (each with a synthetic task).
 * Reports:
 ...
 * NDJSON line `{"name":"b5.deep-tracking",...}` — the runner parses the LAST
 * JSON line on stdout.
 *
 * Run standalone:
...
function main(): void {
	...
	for (const subagents of [1, 10, 50]) {
		...
		// (spawn, progress xN, transition, message_end). bytes/4 is the
		// standard token heuristic.
		...
		console.log(
			`b5 n=${subagents}: graph=${round(graphBuildMs)}ms (${graph.nodes.length}N/${graph.edges.length}E, ${graphBytes}B) metrics=${round(metricsMs)}ms estTok/sub=${estTokensPerSubagent}`,
		);
	}
