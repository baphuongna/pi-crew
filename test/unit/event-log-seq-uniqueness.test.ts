...
describe("B7: cross-path seq uniqueness", () => {
	...
	it("explicit pre-assigned seqs don't collide with auto-assigned ones", async () => {
		...
		// FIX (CI flake): use only sync + async paths here. The previous version
		// used appendEventBuffered (20ms timer), and under --test-force-exit
		// the runner occasionally cancelled the test after only ~8ms — before
		// the buffered timer could fire and resolve the queued promise.
		// Tests 1 and 2 above already cover the sync/async/buffered 3-path mix,
		// so this test focuses on the explicit-vs-auto seq assignment contract.
		...
		// Mix explicit and auto assignments across sync + async paths.
		const promises: Promise<unknown>[] = [
			Promise.resolve(appendEvent(eventsPath, ev1)),
			appendEventAsync(eventsPath, auto1),
			appendEventAsync(eventsPath, auto2),
