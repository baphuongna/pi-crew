...
test("discoverProviderExtensions: skips non-npm specs (local paths, git)", () => {
	const { settingsPath, npmBase, cleanup } = makeFakeRegistry();
	try {
		writePackage("pi-commandcode-provider", npmBase, ["./index.ts"]);
		fs.writeFileSync(
			settingsPath,
			JSON.stringify({
				packages: ["../../source/my_pi/pi-crew", "npm:pi-commandcode-provider", "git:github.com/x/y"],
			}),
			"utf-8",
		);
		const result = discoverProviderExtensions(settingsPath);
		assert.equal(result.length, 1, "only the npm: spec should resolve");
		assert.equal(result[0].spec, "npm:pi-commandcode-provider");
	} finally {
		cleanup();
	}
