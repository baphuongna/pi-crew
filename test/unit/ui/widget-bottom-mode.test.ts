/**
 * Unit tests for `widgetPlacement: "bottom"`: the placement maps to pi's
 * `belowEditor` widget slot. (The crew-vibes footer-dock variant was retired
 * 2026-09-13 — pi's native footer is always shown; see UI-review option 1.)
 */
...
test("bottom placement installs the widget in the belowEditor slot", () => {
	...
	try {
		...
		assert.equal(activeInstalls.length, 1, "widget slot used");
...
test("aboveEditor keeps its own slot", () => {
...
test("switching to a no-run state hides the widget", () => {
	...
	try {
		...
		assert.equal(installCallsFor(harness.widgetCalls, "pi-crew-active").length, 1, "installed while a run exists");

		// Second tick: nothing left to display (no runs) — widget must be cleared.
		...
		assert.ok(cleared.length >= 1, "widget cleared when there is nothing to paint");
	} finally {
		removeTrackedTempDir(cwd);
	}
});

test("stopCrewWidget clears the slot", () => {
	...
	try {
		...
		updateCrewWidget(harness.ctx, state, { widgetPlacement: "bottom" }, undefined, undefined, [manifest]);
		assert.equal(installCallsFor(harness.widgetCalls, "pi-crew-active").length, 1);
