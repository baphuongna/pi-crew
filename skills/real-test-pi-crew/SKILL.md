...
| Async run tưởng "luôn headless" | **KHÔNG còn** (2026-08-27): bỏ hard-gate async → surface theo env + `runtime.surface.*`; async vẫn detect mux | Test với live mux (env herdr/tmux) + `visibleAgents` set; kiểm `worker.surface_gate_blocked` (chỉ còn mode/depth/cap/role/no-mux) |
