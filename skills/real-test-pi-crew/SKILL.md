...
- ⚠️ Prune on the real root sees only ~11 finished runs while disk has ~100+ run dirs — index-scoped listing (the F-L1 union/XOR family). Do not assume prune keeps=20 protects old disk runs.
...
- ⚠️ **Scheduled-job spawn storm (post-mortem 2026-09-21)**: a registered interval job (correct nextRun) fired in bursts every ~15min, each fire dispatching runs via the in-session executor; ~104 garbage runs + 8 manifest-less spawn corpses; ended only by job removal + `pkill -f '[b]ackground-runner.ts --cwd <root>'` (bracket pattern — self-match trap). Cleanup: forget-loop via handleForget script; manifest-less dirs are invisible to the product (rm directly). Fire events are emitted to `pi.events` only, NOT persisted — if you need the trigger, add persistence first.
...
| Runs keep appearing every ~15min under one scheduled job; ~1-2 runs/sec during bursts; children of YOUR pi process | In-session scheduler executor dispatch loop (spawn storm). Contain: remove the job FIRST, then `pkill -f '[b]ackground-runner.ts --cwd <root>'`; cleanup via handleForget loop; manifest-less spawn corpses are product-invisible (rm directly) | If bursts continue after job removal, the executor chain is still live — the pkill is what actually stops it (measured 2026-09-21: spawns continued 4min past removal until pkill) |
