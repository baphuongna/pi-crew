 ...
 * Modeled on the (removed in Phase 1.7) ScheduleStore coordination pattern but with atomicWriteJson +
	...
	save(state: GoalLoopState, eventsPath?: string): void {
		...
		try {
			...
			if (eventsPath) {
				...
				// goal.state_changed is read back synchronously by CAS tests and
				// goal status display; low-frequency transition event.
